...
export default function Analytics() {
  ...
  const toggleSelection = (item) => {
    ...
    } else if (selectedFilter === "Zone") {
      selectedArray = filter.zone.includes(item)
        ? filter.zone.filter((i) => i !== item)
        : [...filter.zone, item];
      setfilter({ ...filter, zone: selectedArray });
  ...
  return (
    ...
    >
      {error ? (
        <ErrorPage onRetry={fetchResource} />
      ) : loading ? (
        <View
          style={{
            height: 600,
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <View style={styles.sidebar}>
              ...
              <TouchableOpacity
                onPress={() => setfilter({ ...filter, selectedFilter: "Zone" })}
                style={styles.tab}
              >
                <Text
                  style={
                    filter.selectedFilter === "Zone"
                      ? styles.activeTabText
                      : styles.tabText
                  }
                >
                ...
                {(filter.selectedFilter === "Year"
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.badge,
                      (filter.selectedFilter === "Year" &&
                        filter.year.includes(item)) ||
                      (filter.selectedFilter === "Batch" &&
                        filter.batch.includes(item)) ||
                      (filter.selectedFilter === "Designation" &&
                        filter.designation.includes(item)) ||
                      (filter.selectedFilter === "Zone" &&
                        filter.zone.includes(item)) ||
                      (filter.selectedFilter === "Status" &&
                        filter.status.includes(item))
